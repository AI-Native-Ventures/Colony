/// Turns bare and autolinked URLs in message text into Markdown links.
///
/// The markdown renderer only builds an anchor for `[label](url)`, so a
/// pasted `buzz://message?...` permalink would otherwise render as plain
/// text and never reach the link builder that draws it as a chip.
///
/// Inline code and fenced blocks are left untouched: a URL inside backticks
/// is being shown, not linked.
const _markdownDelimiters = ['***', '___', '**', '__', '~~', '*', '_'];

// Only the `buzz://` forms Colony actually parses. Upstream also matches
// `pr?`, `issue?` and `repo?` entity permalinks, which Colony has no parser
// for: matching them would turn a URL into a link that renders as a chip and
