...
class ActivityNotifier extends AsyncNotifier<HomeFeedResponse> {
  ...
  Future<HomeFeedResponse> _fetch() async {
    ...
    // here replaces the four simultaneous websocket history subscriptions
    // that otherwise compete with channel and preference startup sync.
  ...
  /// One batched HTTP read, falling back to websocket history per filter.
  ///
  /// The fallback runs in bounded groups rather than all at once: the whole
  /// point of batching is to stop this feed from opening several simultaneous
  /// history subscriptions during startup, and an unbounded fallback would
  /// reintroduce exactly that on the first failure.
