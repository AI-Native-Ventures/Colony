...
///
/// The longest option sits exactly on the relay's `MAX_INVITE_TTL_SECS`
/// ceiling, so nothing here can be rejected for being out of range.
...
/// A relay identity that can be invited to a community.
@immutable
class CommunityInviteRecipient {
  /// Creates a recipient for [pubkey] and optional profile metadata.
  const CommunityInviteRecipient({
...
CommunityInviteRecipient? _recipientFromEvents(
  List<NostrEvent> events,
  String pubkey,
) {
  NostrEvent? latest;
  for (final event in events) {
    if (event.kind != 0 || event.pubkey.toLowerCase() != pubkey) continue;
    if (latest == null || event.createdAt > latest.createdAt) latest = event;
  }
  if (latest == null) return null;
  final profile = ProfileData.fromEvent(latest);
  return CommunityInviteRecipient(
...
/// metadata falls back to a pubkey-only recipient instead of blocking the
/// invite. Relay failures still surface as errors and malformed inputs return
/// null.
final communityInviteProfileProvider = FutureProvider.autoDispose
    .family<CommunityInviteRecipient?, String>((ref, pubkey) async {
      ...
      return _recipientFromEvents(events, normalized) ??
          CommunityInviteRecipient(pubkey: normalized);
    });
