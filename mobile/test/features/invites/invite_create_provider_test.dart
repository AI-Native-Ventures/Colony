...
void main() {
  ...
  test('submits one kind:9030 event per invited pubkey', () async {
    final keys = nostr.Keys.generate();
    final invitee = nostr.Keys.generate();
    ...
    await service.inviteMembers(
      // The same identity in both encodings must collapse to one event.
      pubkeys: [invitee.npub, invitee.public, 'not-a-pubkey'],
      role: CommunityMemberRole.member,
    );

    expect(session.published, hasLength(1));
    expect(session.published.single.kind, EventKind.relayAdminAddMember);
    expect(session.published.single.tags, [
      ['p', invitee.public],
      ['role', 'member'],
    ]);
  });
  ...
  test('surfaces the relay error message from a rejected mint', () async {
    final keys = nostr.Keys.generate();
    ...
    await expectLater(
      service.mintInvite(ttlSeconds: 86400, maxUses: null),
      throwsA(
        isA<Exception>().having(
          (error) => error.toString(),
          'message',
          contains('only relay owners and admins can create invites'),
        ),
      ),
    );
  });

  test('refuses to mint once the active community has changed', () async {
    final keys = nostr.Keys.generate();
    var requests = 0;
    final client = http_testing.MockClient((_) async {
      requests++;
