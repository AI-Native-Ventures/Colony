...
async fn authorize_workflow_read(
    ...
    // Colony's bridge auth also carries the verified signed timestamp, which
    // `enforce_relay_membership` needs to judge an auth tag against the moment
    // the request was signed rather than the moment it arrived.
