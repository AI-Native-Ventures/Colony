...
pub(crate) fn p_gated_filters_authorized(filters: &[Filter], authed_pubkey_hex: &str) -> bool {
    ...
    filters.iter().all(|filter| {
        ...
        // side for that shape, so a filter that names KIND_USAGE_RECORD and
        // is scoped to the reader's own pubkey is authorized.
        filter.kinds.as_ref().is_some_and(|ks| {
            ks.iter()
                .any(|kind| (kind.as_u16() as u32) == KIND_USAGE_RECORD)
...
/// any kind listed in `RESULT_GATED_KINDS`).
///
...
mod tests {
    ...
    #[test]
    fn discovery_receipts_require_matching_p_even_with_known_id() {
        let p_tag = SingleLetterTag::lowercase(Alphabet::P);
        ...
        for kind in [
            buzz_core::kind::KIND_DISCOVERY_RECEIPT,
            buzz_core::kind::KIND_DISCOVERY_WORKER_RECEIPT,
        ] {
            let receipt_kind = nostr::Kind::Custom(kind as u16);
            let ids_only = Filter::new()
                .kind(receipt_kind)
