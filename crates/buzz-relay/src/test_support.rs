//! Test-only helpers shared by the relay's unit tests.
//!
//! Upstream also exposes a `database_url()` helper here; Colony's PostgreSQL
//! tests resolve their own URL, so only the isolated-child runner is carried.
