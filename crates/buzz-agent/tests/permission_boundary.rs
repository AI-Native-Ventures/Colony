...
/// A client that never answers must not wedge the turn.
///
/// The broker's own timeout unit test proves the correlation entry is removed
/// and the slot freed; this proves the turn-level consequence end to end, which
/// a subprocess is the only place to observe: the tool never runs, the prompt
/// still returns `end_turn`, and it returns within the permission deadline
/// rather than hanging until the outer tool or LLM budget fires.
///
/// Colony runs provisioned employees on buzz-agent behind the credits gateway,
/// where a stalled client is a real failure mode, not a hypothetical one.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_unanswered_permission_times_out_without_invoking_tool_or_hanging() {
    let log = call_log_path("timeout");
    let llm = spawn_capturing_llm(vec![shell_call("tc1"), openai_text("gave up")]).await;
    // A short permission deadline keeps the test fast; every other budget stays
    // well above it so a pass cannot be some other timeout firing first.
    let mut h =
        Harness::spawn_with_env(&llm.url, &[("BUZZ_AGENT_PERMISSION_TIMEOUT_SECS", "1")]).await;
    let sid = init(
        &mut h,
        1,
        "/tmp",
    ...
    // `None` records the request and deliberately never responds to it.
    let (resp, requests) = drive(&mut h, &sid, "go", |_| None).await;
    let elapsed = started.elapsed();

    assert_eq!(requests.len(), 1, "the agent still asks exactly once");
    assert_eq!(
        stop_reason(&resp),
        "end_turn",
        "an unanswered permission must fail the tool call, not the turn"
    );
    assert!(
        call_log_lines(&log).is_empty(),
        "an unanswered permission must never reach MCP"
    );
    assert!(
        elapsed < Duration::from_secs(5),
        "the turn must end on the permission deadline, not hang: took {elapsed:?}"
    );
    h.shutdown().await;
}
