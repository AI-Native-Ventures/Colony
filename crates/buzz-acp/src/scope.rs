...
/// Placeholder in `base_prompt.md` marking where the policy-selected session
/// model is spliced in. Colony's base prompt keeps the section in its authored
/// position rather than appending it after every other section.
pub(crate) const SESSION_MODEL_MARKER: &str = "<!-- SESSION_MODEL -->";

/// Operator policy controlling how ACP provider sessions are scoped.
///
/// Selected via `--session-policy` / `BUZZ_ACP_SESSION_POLICY`. Upstream ships
/// this dark with a `channel` default; Colony has keyed ACP sessions per
/// (channel, thread root) since `cb511390ac`, so its CLI default is `thread`
/// and `channel` is the rollback. The `Default` impl below stays `Channel` so
/// struct-literal defaults keep the conservative value.
...
/// True for a 64-character hex event id (either case).
...
impl SessionPolicy {
    ...
    pub(crate) fn append_session_model(self, base_prompt: &str) -> String {
        ...
        // Colony's base prompt carries a placeholder so the session model keeps
        // its authored position (right after "Your place in the company")
        // instead of landing at the end of a long prompt. Callers that pass a
        // prompt without the marker - upstream tests and custom instructions -
        // get the appended form.
        if let Some((head, tail)) = base_prompt.split_once(SESSION_MODEL_MARKER) {
            return format!("{}{}{}", head, session_model.trim_end(), tail);
        }
...
impl SessionScope {
    ...
    ///
    /// Thread roots are resolved with [`parse_thread_tags`]. A marker id that is
    /// not a 64-character hex event id is ignored (treated as top level) so a
    /// malformed tag cannot mint a session key. Colony's resolver treats a lone
    /// `root` marker with no `reply` as a reply into that thread, where upstream
    ...
    pub fn derive(policy: SessionPolicy, channel_id: Uuid, is_dm: bool, event: &Event) -> Self {
        ...
        // A thread scope is a session key, so it must be a real event id: a
        // malformed `e` tag falls back to a top-level scope rooted at the
        // trigger rather than minting a session keyed on junk.
        let root_event_id = match parse_thread_tags(event).root_event_id {
            Some(root) if is_event_id(&root) => root,
            _ => event.id.to_hex(),
        };
        Self::Thread {
            channel_id,
...
mod tests {
    ...
    #[test]
    fn session_model_is_spliced_once_and_matches_policy() {
        let base = include_str!("base_prompt.md");
        assert!(!base.contains("## Session Model"));
        assert!(
            base.contains(SESSION_MODEL_MARKER),
            "base_prompt.md must keep the session-model placeholder"
        );
        for policy in [SessionPolicy::Channel, SessionPolicy::Thread] {
            let prompt = policy.append_session_model(base);
            // Spliced at the marker, not appended: the marker is gone, the
            // section lands in its authored position, and the rest of the
            // prompt still follows it.
            assert!(!prompt.contains(SESSION_MODEL_MARKER));
            assert_eq!(prompt.matches("## Session Model").count(), 1);
            let model_at = prompt.find("## Session Model").unwrap();
            let cli_at = prompt.find("## The `buzz` CLI").unwrap();
            assert!(model_at < cli_at);
    ...
    #[test]
    fn session_model_is_appended_when_no_marker_is_present() {
        let base = "Custom base instructions.";
        for policy in [SessionPolicy::Channel, SessionPolicy::Thread] {
            let prompt = policy.append_session_model(base);
    ...
    fn direct_reply_to_root_scopes_to_that_root() {
        ...
        // Colony's NIP-10 resolver (`queue::parse_thread_tags`) treats a lone
        // `root` marker as a direct reply into that thread, where upstream
        // treats it as top level. The scope follows the resolver, so the turn
        // joins the existing thread's session instead of opening a new one.
        let scope = SessionScope::derive(SessionPolicy::Thread, ch, false, &ev);
        assert_eq!(
            scope,
            SessionScope::Thread {
    ...
    fn malformed_thread_tag_falls_back_to_top_level() {
        let ch = Uuid::new_v4();
        // A non-64-hex marker id is rejected by `derive`, so the event is
        // treated as top level (rooted at its own id) rather than minting a
        // session keyed on a malformed thread root.
        let ev = event_with_tags(vec![vec![
            "e".into(),
