...
use crate::usage::{
    StandardAdapterKind, StandardPromptUsage, StandardUsageTracker, TurnUsage, UsageTracker,
};
...
pub struct AcpClient {
    ...
    ///
    /// Forced to `None` when [`Self::cumulative_prompt_usage`] is set: Colony's
    /// Codex usage adapter already records that turn's usage into
    /// `goose_usage`, and a second reader would be a duplicate source for the
    /// same numbers.
...
impl AcpClient {
    ...
    pub async fn spawn(
        ...
        Ok(Self {
            ...
            // Exactly one reader per turn: Colony's Codex adapter path owns the
            // usage whenever it was launched, so the standard reader stands down.
            standard_adapter: if prepared_codex_usage.is_some() {
                None
            } else {
                standard_adapter
            },
    ...
    fn parse_prompt_response(
        ...
        if let Some(adapter) = self.standard_adapter {
            match serde_json::from_value::<StandardPromptUsage>(result["usage"].clone()) {
                Ok(usage) => self
                    .standard_usage
...
mod tests {
    ...
    async fn spawn_named_script(name: &str, script: &str) -> (AcpClient, std::path::PathBuf) {
        ...
        let client = AcpClient::spawn(path.to_str().expect("utf8 path"), &[], &[], false, None)
            .await
