...
/// The absolute working directory every managed skill and launcher path is
/// resolved against. A relative cwd would let a generated Pi launcher point at
/// a directory that moves with the process.
...
mod inbound_author_gate {
    ...
    impl InboundAuthorGate {
        ...
        /// Apply the gate to a bare event whose DM status the caller already
        /// knows. Colony's Ask inbox is a second inbound listener: an ask is
        /// addressed to an agent and has no channel, so it cannot go through
        /// `evaluate_listener_event`, but it must still resolve the effective
        /// author and apply the same policy rather than calling the raw
        /// author policy directly.
        pub(crate) async fn evaluate_bare_event(
            &self,
            event: &nostr::Event,
            respond_to: &RespondTo,
            allowlist: &HashSet<String>,
...
async fn refresh_relay_self(
    ...
    match rest_client.relay_self().await {
        Ok(Some(pubkey)) => (Some(pubkey.to_hex()), true),
        Ok(None) => {
            tracing::warn!(
                %context,
...
struct SteerAckEvent {
    channel_id: Uuid,
    /// Session scope of the steered event, matching [`pool::TaskMeta::scope`]
    /// of the task the steer targeted. The queue-side withhold/release, the
    /// deadline extension and the fallback signal all target this, not the
    /// whole channel, so a sibling thread's turn is never disturbed.
                ...
                ask_event = async {
                    ...
                    match ask_event {
                        Some(event) => {
                            ...
                            // Same gate as the channel listener, so a
                            // relay-signed workflow ask is judged as its
                            // workflow owner. An ask has no channel, so DM
                            // hardening does not apply.
                            let decision = author_gate_ctx
                                .evaluate_bare_event(
                                    &event,
                                    &config.respond_to,
                                ...
                                .await;
                            let author = decision.effective_author;
                ...
                buzz_event = relay.next_event() => {
                    ...
                    match buzz_event {
                        Some(buzz_event) => {
                            ...
                            // The gate resolves the EFFECTIVE author before
                            // applying policy: a relay-signed workflow message
                            // that carries verified `buzz:workflow` provenance
                            // and names this agent is attributed to the workflow
                            // owner. Colony's owner-only gate then runs against
                            // that owner, so a workflow wakes an agent exactly
                            // when its owner could have. Everything else keeps
                            // the raw signer. DM hardening is inside the gate:
                            // channel type resolves fail-closed to DM, so
                            // allowlist/anyone modes cannot be exercised by
                            // non-owner authors inside DMs.
                            // The gate resolves the EFFECTIVE author before
                            // applying policy: a relay-signed workflow message
                            // that carries verified `buzz:workflow` provenance
                            // and names this agent is attributed to the workflow
                            ...
                            // non-owner authors inside DMs.
                            //
                            // The event is moved into the gate and can only be
                            // recovered from the authorized capability, so a
                            // local `allowed = true` at this call site no longer
                            // type-checks. `production_listener_boundaries_apply_workflow_owner_policy`
                            // pins this exact callable.
                            ...
                            .await
                                ...
                                // INFO-level "not answering" logging lives in the
                                // gate, which is the only place that knows both
                                // the raw signer and the effective author.
                                continue;
                            };
                            let (buzz_event, effective_author) = authorized.into_parts();
                            ...
                            // The mode gate below compares against the owner, so
                            // it reads the same effective author the policy gate
                            // just authorized: a workflow message steers as its
                            // owner, not as the relay key that signed it.
                            ...
                            // tags. Everything downstream (queue partition,
                            // provider session, steer target, delivery ledger)
                            // carries this resolved scope instead of re-deriving
                            // it from whichever event happens to be last.
                            ...
                            if accepted && queue.is_scope_in_flight(&session_scope)
                                ...
                                if let Some(signal) = signal {
                                    ...
                                    let in_flight_matches = pool
                                        .in_flight_scope(buzz_event.channel_id)
                                        .is_some_and(|task_scope| task_scope == session_scope);
                                        ...
                                        && !in_flight_matches
                                    {
                                        tracing::info!(
                                            ...
                                            incoming_scope = %session_scope.telemetry_label(),
    ...
    loop {
        ...
        let abort_handle = pool.join_set.spawn(async move {
            pool::run_prompt_task(
                ...
                Some(control_rx),
                task_turn_id,
                PromptSource::Channel(task_scope),
            )
            .await;
        });
...
mod owner_control_command_tests {
    ...
    async fn observer_channel_controls_reject_sibling_sessions_without_signalling() {
        ...
        // Only the cancel frame echoes requestId here: Colony's switch_model
        // frame carries modelId instead (it has no request-id correlator).
        assert_eq!(results[0].payload["type"], "cancel_turn");
        assert_eq!(results[0].payload["requestId"], "pick-1");
        assert_eq!(results[1].payload["type"], "switch_model");
        assert_eq!(results[1].payload["modelId"], "new-model");
        assert_eq!(
            rx_a.try_recv(),
