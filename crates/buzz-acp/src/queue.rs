...
impl EventQueue {
    ...
    pub fn flush_next(&mut self) -> Option<FlushBatch> {
        ...
        // A recovered scoped request must not merge with a later message. The
        // recovery is per session scope, so under the thread policy only the
        // thread that was interrupted is re-dispatched.
        if let Some(scope) = self
            ...
            .map(|(scope, _)| scope.clone())
        {
            let recovered = self.cancelled_batches.get_mut(&scope)?;
            let index = recovered
                .iter()
...
pub fn parse_thread_tags(event: &Event) -> ThreadTags {
    ...
    // Colony diverges from `ThreadMarkers::resolve()` in exactly one case: an
    // event carrying a lone `root` marker and no `reply`. Upstream reads that
    // as top-level, which is right for relay ingest and is what the ingest path
    // uses. On the ACP side it would open a NEW session for a message that a
    // client wrote into an existing thread, so here a lone `root` anchors to
    // that thread (see `scope::tests::direct_reply_to_root_scopes_to_that_root`).
    let (root_event_id, parent_event_id) = match markers.resolve() {
        Some((root, parent)) => (Some(root), Some(parent)),
        None => match markers.root.clone() {
...
/// Format a `[Context]` hints section from the resolved session scope and turn routing.
///
...
fn format_context_hints(
        ...
        .or(thread_tags.root_event_id.as_deref())
    {
        ...
        s.push_str(&format!("\nThread root: {root}"));
        if let Some(parent) = thread_tags.parent_event_id.as_deref() {
            if parent != root {
                s.push_str(&format!("\nParent: {parent}"));
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_parse_thread_tags_root_only_anchors_to_that_root() {
        // Only a `root` marker, no `reply`. Upstream reads this as top-level to
        // match relay ingest; Colony anchors it to that thread, because on the
        // ACP side the alternative is opening a fresh session for a message a
        // client wrote into an existing thread. `scope::tests::
        // direct_reply_to_root_scopes_to_that_root` pins the same rule from the
        // session side; the ingest path still uses the shared resolver.
        let root = "a".repeat(64);
        let event = make_event_with_tags(
            "root only",
            vec![vec!["e".into(), root.clone(), "".into(), "root".into()]],
        );
        let tags = parse_thread_tags(&event);
        assert_eq!(tags.root_event_id.as_deref(), Some(root.as_str()));
        assert_eq!(tags.parent_event_id.as_deref(), Some(root.as_str()));
...
mod reply_model_queue_tests {
    ...
    /// Conversation scope for a channel — the scope these tests exercise.
    fn conv(channel_id: Uuid) -> SessionScope {
        SessionScope::Conversation { channel_id }
    }
    ...
    fn recovered_scoped_request_preserves_other_cancelled_messages() {
        ...
        let entries = [("normal", false), ("scoped", true)]
            ...
            .collect();
        queue.cancelled_batches.insert(conv(channel), entries);
        queue
            .cancel_reasons
            .insert(conv(channel), CancelReason::Steer);
