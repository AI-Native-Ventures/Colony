//! Focused PostgreSQL tests for the database runtime.
...
async fn catalog_action_same_key_commits_exactly_one_global_batch() {
    ...
    let (action, head, receipt, original_action_event_id) = match (first_outcome, second_outcome) {
        (
...
async fn catalog_action_failures_roll_back_claim_and_all_batch_events() {
    ...
    let receipt_failure = catalog_batch(&relay_keys, &receipt_handle, base + 20, "receipt-failure");
...
fn read_budget_zero_disables_and_large_values_clamp_to_staleness() {
    ...
    assert_eq!(
        ...
        Some(crate::replica_fence::FENCE_STALENESS),
...
fn for_query_predicate_truth_table() {
    ...
    let pinned_with_until = {
        let mut q = crate::event::EventQuery::for_community(community);
...
async fn routed_request_holds_one_snapshot_across_page_and_aux() {
    ...
    let visible_elsewhere = crate::event::query_events(&replica, &control)
        .await
...
async fn head_fetch_routes_by_configured_budget() {
    ...
    db.fence().force_open_for_tests(chrono::Utc::now());
    let cid = CommunityId::from_uuid(community);
    let head_contents = |w: &crate::thread::ChannelWindow| -> Vec<String> {
        w.rows
            .iter()
            .map(|r| r.stored_event.event.content.clone())
            .collect()
...
async fn channel_cursor_above_fence_stays_on_writer_preventing_middle_hole() {
    ...
    let contents = |w: &crate::thread::ChannelWindow| -> Vec<String> {
        w.rows
            .iter()
            .map(|r| r.stored_event.event.content.clone())
            .collect()
...
async fn armed_pool_rejects_old_channel_inserts_through_public_api() {
    ...
    let err = crate::event::insert_event(&db.pool, cid, &old, Some(channel))
        .await
    ...
    let err = crate::event::insert_event_with_thread_metadata(
        &db.pool,
        cid,
        &old2,
        Some(channel),
    ...
    crate::event::insert_event(&db.pool, cid, &fresh, Some(channel))
        .await
