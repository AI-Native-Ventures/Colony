...
pub(crate) enum ReaderOperation {
    Bootstrap,
    /// Upstream constructs this from the replica-routed relay-membership read.
    /// Colony's `Db::is_relay_member` still reads the writer directly, so the
    /// variant is currently only reachable through the metric contract in
    /// [`POOL_ACQUIRE_VALID_PAIRS`].
    #[allow(dead_code)]
    Authorization,
    SubscriptionHistory,
}
