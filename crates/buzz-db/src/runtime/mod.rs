//! Database runtime: pool construction, read routing, sessions, and metrics.
//!
//! Runtime concerns own pool construction, writer/replica routing, transaction
//! creation, session invariants, metrics, health support, and migrations. They
//! are deliberately kept out of the domain stores under [`crate::store`].
...
pub struct Db {
    ...
    ///
    /// Starts closed; a background probe ([`crate::replica_fence::run_probe`])
    ...
    pub(crate) fence: std::sync::Arc<crate::replica_fence::ReplicaFence>,
    ...
    /// function ([`crate::replica_fence::AURORA_IDENTITY_FN`]) — probed
    ...
    pub async fn query_events(&mut self, q: &EventQuery) -> Result<Vec<StoredEvent>> {
        let degraded = match &mut self.inner {
            ReadSessionInner::Replica { tx, writer } => {
                match crate::event::query_events_on(tx, q).await {
            ...
            ReadSessionInner::Writer(pool) => return crate::event::query_events(pool, q).await,
        };
        ...
        crate::event::query_events(&degraded, q).await
    }
...
pub(crate) enum RouteDecision {
    ...
    Replica(
        ...
        crate::replica_fence::TokenEntry,
        &'static str,
    ),
...
pub(crate) mod route_proof {
    ...
    impl ChannelScoped {
        ...
        /// builders — the SELECT builder (`crate::event::query_events_on`) and the
        /// COUNT builder (`crate::event::count_events`) pin identically; if the
...
impl RoutePredicate {
    ...
    /// unchanged.
    pub(crate) fn for_query(q: &crate::event::EventQuery, routing_enabled: bool) -> Self {
        if !routing_enabled {
...
fn read_budget_from_ms(ms: u64) -> Option<Duration> {
    match ms {
        0 => None,
        ms => Some(Duration::from_millis(ms).min(crate::replica_fence::FENCE_STALENESS)),
    }
}
...
pub struct DbConfig {
    ...
    /// [`crate::replica_fence::FENCE_STALENESS`] are clamped to it: an entry older
...
impl Db {
    ...
    pub async fn new(config: &DbConfig) -> Result<Self> {
        ...
        Ok(Self {
            ...
            read_pool,
            read_max_connections,
            fence: std::sync::Arc::new(crate::replica_fence::ReplicaFence::new()),
    ...
    /// [`crate::replica_fence::ReplicaFence::force_open_for_tests`] (see
    /// [`Db::fence`]).
    ...
    pub fn fence(&self) -> &std::sync::Arc<crate::replica_fence::ReplicaFence> {
        &self.fence
    }
    ...
    /// catalog shape ([`crate::replica_fence::verify_floor_guard_catalog`]) and
    /// observed semantics through this exact pool
    /// ([`crate::replica_fence::verify_floor_guard_behavior`]).
    ///
