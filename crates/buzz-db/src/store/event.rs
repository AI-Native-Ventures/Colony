...
use buzz_core::kind::{
    ...
    KIND_HUDDLE_STARTED, KIND_TASK, SHARED_GATED_KINDS,
};
...
/// A durable ledger action claim, keyed by community and idempotency key.
#[derive(Debug)]
pub struct LedgerActionClaim {
    /// The action event that won the claim.
    pub action_event_id: Vec<u8>,
    /// The book head it produced.
    pub head_event_id: Vec<u8>,
    /// The receipt the relay signed for it.
    pub receipt_event_id: Vec<u8>,
}

/// Outcome of brokering one owner-signed ledger action.
#[derive(Debug)]
#[allow(clippy::large_enum_variant)] // Preserve direct StoredEvent handoff for relay fan-out.
pub enum LedgerActionApply {
    /// The action, book head, and receipt committed together.
    Applied {
        /// Stored owner-signed action.
        action: StoredEvent,
        /// Stored relay-authored book head.
        head: StoredEvent,
...
pub(crate) async fn query_events_on(
    ...
    // Shared-gated visibility pushdown: exclude shared-gated kind events that
    // are neither authored by the reader nor explicitly shared.  Applied
    // BEFORE ORDER/LIMIT so that a page of newer private events does not push
    // visible shared ones off the end of the result set (the catalog query
    // pattern).
    //
    // Clause: AND (kind NOT IN (30175,30193) OR pubkey = $reader OR
    // tags @> '[["shared","true"]]')
    //
    ...
    if let Some(reader_bytes) = &q.shared_gated_reader {
...
impl Db {
    ...
    pub async fn count_events_routed(&self, path: &'static str, q: &EventQuery) -> Result<i64> {
        match self.route_read(path, RoutePredicate::Bounded).await {
            ...
            RouteDecision::Writer => crate::event::count_events(&self.pool, q).await,
        }
    }
...
mod tests {
    ...
    async fn reminder_claim_and_release_are_confined_to_their_community() {
        ...
        assert!(
            crate::reminder::release_due_reminder(&pool, community_a, &id, created_at, stamp)
                .await
                .expect("release A"),
