...
mod tests {
    ...
    /// Single-ownership guard for the community extraction: every moved
    /// implementation, record and test must live in this module and nowhere
    /// else. Upstream also asserts one `datastore_span` per operation; Colony's
    /// community operations carry no spans (none of them ever did in `lib.rs`),
    /// so that half is omitted rather than asserted against zero.
    #[test]
