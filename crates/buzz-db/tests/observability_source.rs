...
fn p0_pool_acquisitions_use_typed_operation_pairs_without_other() {
    ...
    let get_channel = channel
        ...
        .1
        .split_once("/// Lists channels in a community")
        .expect("channel lookup helper must precede channel listing")
        .0;
    ...
    // Upstream's ingest validates Huddle lifecycle events and is the second
    // caller of the event-write variant. Colony's ingest has no Huddle
    // lifecycle validation, so the audio handler is the only caller here.
    ...
    // Colony's upsert is `ON CONFLICT DO UPDATE`, which always returns a row,
    // so it reads with `fetch_one`; upstream's tombstone-filtered variant uses
    // `fetch_optional`. Either way the read must run on the attributed
    // connection rather than the bare pool.
    assert!(
        ensure_community.contains("fetch_one(&mut *connection)")
            || ensure_community.contains("fetch_optional(&mut *connection)")
    );
    ...
    let fence_boot = runtime
        ...
        .1
        .split_once("/// Whether a distinct read-replica pool is configured")
        .expect("boot fence verification must precede read-pool plumbing")
        .0;
