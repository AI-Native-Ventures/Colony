...
if [[ "$TARGET" != *windows* ]]; then
    ...
    BUILD_HINT="cargo build --release -p buzz-acp -p buzz-agent -p buzz-backend-kubernetes -p buzz-browser -p buzz-dev-mcp -p git-credential-nostr -p buzz-cli"
else
    BUILD_HINT="cargo build --release -p buzz-acp -p buzz-agent -p buzz-browser -p buzz-dev-mcp -p git-credential-nostr -p buzz-cli"
fi
