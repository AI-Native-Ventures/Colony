...
grep -q '^APP_DISPLAY_NAME = Colony (Fix_Thing-2)$' "$ios" \
...
grep -Eq "^APP_DISPLAY_NAME = Colony \([A-Za-z0-9._-]+\)$" "$ios" \
...
grep -q '^APP_DISPLAY_NAME = Colony$' "$release_xcconfig" \
  && pass "Release.xcconfig keeps the production display name" \
  || fail "Release.xcconfig must keep APP_DISPLAY_NAME = Colony"
...
grep -q 'resValue("string", "app_name", "Colony")' "$gradle" \
  && pass "Gradle default app_name stays Colony" \
