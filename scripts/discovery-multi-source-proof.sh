...
cleanup_discovery_proof_db() {
  PGPASSWORD=buzz_dev dropdb --if-exists --force \
    --host localhost --port "${HARNESS_PG_PORT}" --username buzz "${DISCOVERY_PROOF_DB}"
}
trap cleanup_discovery_proof_db EXIT
PGPASSWORD=buzz_dev createdb \
  --host localhost --port "${HARNESS_PG_PORT}" --username buzz "${DISCOVERY_PROOF_DB}"
BUZZ_TEST_DATABASE_URL="postgres://buzz:buzz_dev@localhost:${HARNESS_PG_PORT}/${DISCOVERY_PROOF_DB}" \
