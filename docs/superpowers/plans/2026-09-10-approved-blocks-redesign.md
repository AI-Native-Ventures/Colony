...
## Corrected-head relay evidence

Run `34496714266` at `7b16fc12b0526fc30e685d8d6fca150b2857378c` passed Rust lint, unit tests, Desktop Core, both server builds, all six browser shards, backend integration, agent asks and both desktop relay-integration shards. The Blocks shard again passed all 203 cases on the first attempt, including 62 approved cases and 131 unique captures. The independent live Blocks gate again stored the two action/receipt pairs and executed one mocked native creation command; it encountered no quota rejection. One unrelated terminal buffer-observation test needed a retry, while all relevant media and history cases passed on their first attempt; its cause was not established and no terminal code changed.

The deterministic hosted quota test executed and passed in 6.31 seconds: `test_ws_quota_rejection_retries_identical_event_after_expiry`, one passed, zero failed. The Relay E2E job failed later when `e2e_company_work::seed_live_work_context` received an explicit quota rejection and its fixture treated it as a final domain answer, then waited for a receipt the relay could not have authored. The matching fixture in `e2e_thread_tasks` has the same assumption. These test-only callers need bounded retries of the identical event for explicit quota rejections, while preserving all non-rate-limit conflict, permission and superseding-action behavior. The receipt assertions and production quotas remain unchanged. Production promotion stays held until the corrected fixture run passes every applicable gate.

Both fixtures now import one test-only publishing helper. It preserves their eight-attempt allowance and the client's thirty-second acknowledgement windows, enforces one 242-second overall deadline before every transmission, and honors the relay's retry hint with a small expiry margin. All other acknowledgements return unchanged; intentional quota and permission-negative callers still use the raw test client. Source review and exact-file Rust formatting pass; no local tests or compilation ran. Separate workflows on this head also passed: native first-job `34496714243`, both Electron package/migration jobs `34496714341`, and mesh lifecycle `34496714252`. The fixture correction awaits its own GitHub run.

## Receipt ordering exposed by hosted browser proof

Run `34501638713` at `97334bef790408ae06d0ae6c15ddf332acacec63` passed all six browser shards, Desktop Core, Rust lint and unit tests. All 62 approved scenarios passed on their first attempt, with 131 distinct captures and all 96 manifest/theme/channel/thread hashes matching their 48 proof records. Relevant document, media and history cases passed on their first attempt. The unrelated terminal buffer-observation test again passed on retry, with output visible in its failure capture; no terminal source changed.

The broad Blocks shard reported 202 passed and one flaky case. The saved page context from the failed proposal retry contains both the resolving `Completed` badge and the stale `The action failed` footer in the same proposal. The failure screenshot shows a different proposal; it does not visually establish both labels on the affected proposal. The page context proves the authorized success receipt reached the renderer. The footer selected the globally newest receipt by timestamp and event ID, allowing an older attempt's late or same-second failure to supersede its successful retry. The first attempt retained a screenshot and error context but no trace, so its exact receipt timestamps are unavailable; they must not be inferred from the successful retry trace.

The correction selects the latest action and then that action's latest authorized receipt. Existing signature, instance, manifest and processor checks, within-action receipt ordering and durable attention semantics remain intact. Two deterministic regressions failed against the previous source; the focused eight-test file passed after correction in 2.28 seconds, including equal-timestamp delivery and same-action updates. Independent source review found no blocker. No test timeout was increased. Full suites, compilation and the fresh browser/native verification remain on GitHub, and production promotion remains held.

## Handoff continuation: inline controls, shared states and discovery lifecycle

The transferred work (compact inline reply controls below the draft, both-theme workbook, CSV, catalogue and inline carousel coverage, and a sixteen-case shared-state spec with an E2E-only hold/release seam for Loading) was committed unchanged as `8b914db666`. A read-only source review found no blockers and three hardening items, applied in `db8fc29963`: a release failure in the Loading test's `finally` no longer replaces the original assertion error, an eleventh held block data URL now throws at bridge configuration instead of being silently ignored, and the inline reply model spec waits for the app to drop `messageId` from the thread deep link before evaluating in the page.

`useReplyModelSelection` kept a sticky boolean once Model had been opened, so a later recipient or conversation change queried another teammate's catalogue without the user opening Model. `90153a98f8` keys the authorization on relay, conversation and target, clears it on a real transition, re-authorizes when Model opens, and lets failed-send restoration reinstate the captured same-conversation target. `4b9964c5b8` makes `restore` read live state through a ref, because the composer invokes the send-time closure after a failed send; a stale restore for teammate A can no longer clobber a newer opening for teammate B. Eight node tests in `useReplyModelSelection.test.mjs` cover initial laziness, target switch, conversation switch, post-send, failed-send restore, and the stale-closure cases. Against the original source six fail (both switches, both restore cases, both stale-closure cases); against the closure-based intermediate exactly the stale clobber case fails; all eight pass on the final source and in Desktop Core on runs `34520386287` and `34522834728`. No production file other than the hook changed for this fix.

`origin/develop` through PR #687 was merged in `0bd7e21691`; the only conflict was `CHANGELOG.md`, resolved by listing develop's browser sidecar entry under the v0.16.13 section this branch introduces.

Run `34520386287` at `0bd7e21691` exposed two real spec defects. The Grid fixture omitted `$schema` on `input_schema`, so `validateBlockManifest` rejected it before render in both themes. The inline reply model spec looked for a menu named "Reply model choices", but Radix labels dropdown content by its trigger, so the menu's accessible name is "Reply model"; the dead `aria-label` on the content was removed and the spec corrected. Both landed in `51bee03d9f`. Everything else on that run passed: all 24 core designs, the other fourteen shared-state cases, Desktop Core, Unit Tests, Rust Lint, both server builds, Relay E2E, Backend Integration, Agent Ask, both desktop integration shards, Blocks Live Gate, Native first-job proof, both Electron proofs and Mesh.

Run `34522834728` at `51bee03d9f` passed every non-skipped check: six smoke shards, Desktop Core, Tauri Flags, Unit Tests, Mobile, Web, Security, Rust Lint, both server builds, Relay E2E, Backend Integration, Agent Ask, Relay Suites, Desktop E2E Relay, both desktop integration shards, Blocks Live Gate, plus Native first-job proof `34522834751`, Electron migration and production package proofs `34522834721`, and Mesh `34522834755`. Shard 1 produced 136 approved-design captures and exactly 36 shared-state captures: Grid renders two columns at 1044px in the channel and one column at 296px in the thread with no horizontal overflow; Loading, Empty, Unavailable, Untrusted, Action pending, Action completed and Action failed each captured in both themes and both surfaces, Loading also after release. Shard 4 produced the twelve inline reply model captures (open Model menu, compact chosen values, requested label; both themes, both surfaces) with zero `get_agent_models` calls before the first Model open. The live gate's bounded diagnostics again show four manifest and data queries in `success`, 49 correlated transport frames, none dropped, and one bounded processor completion; this remains normal-path evidence and does not reproduce the earlier `348c43705f` manifest lookup failure, so no runtime retry change was made.

Local execution was limited to the single new test file, exact-file Biome, the desktop `pnpm check` script, and a one-off validator call on the Grid fixture. No local CI, build or browser ran.
