...
CI69c1 Smoke6 run34389938272 passed all four rich-media cases on first attempts: light/dark actual SVG and media playback/seek/retention, narrow thread carousel, and changed accent/mode controls. Its one remaining video fixture failure came from the mock advertising HAVE_ENOUGH_DATA while the poster-lifecycle test had not dispatched its decoded-frame event. Set the mock to HAVE_METADATA, so seeking can restore position while the poster remains until loadeddata. Assertions and product code remain unchanged. The same real runner log confirms the Chrome preflight disabled `/etc/apt/sources.list.d/google-chrome.sources` before successful Playwright system dependency installation.


## CI gallery capability restoration

Smoke4 on 69c1 failed eleven prior gallery cases. Masonry width/crop/row-growth assertions describe the superseded design, while missing image-click expansion, progressive thumbnails, and native copy/context menus exposed real viewing regressions. The shared contained carousel now reuses ProgressiveImage, MediaContextMenu, and spoiler-aware gallery discovery. It registers an ordered list on each mounted trigger in a WeakMap with effect cleanup, preserving repeated occurrences without loading inactive originals. Expanding snapshots only the nearest Markdown/message scope; unrevealed spoiler triggers are excluded before list flattening, and opening/context actions also refuse hidden content. Menus portal to body inline and inside the Radix focus scope when expanded. Closing retains the selected occurrence and restores its originating control.

A thumbnail-first server-render regression failed before the fix, then passed with the metadata carried through. Sixteen focused rendering/actual JSDOM cases pass, including real component opening and native-command arguments (IPC mocked), hidden/revealed first-frame membership, duplicate position and scope isolation. The browser suite retains all eleven cases, with contained carousel geometry replacing masonry geometry. Five tiny generated PNG fixtures now serve the descriptor SHAs with matching dimensions and MIME; screenshots showed these old fixture URLs were previously broken-image placeholders. No local browser/build was run. New visual/focus/browser proof remains the next GitHub CI snapshot.
