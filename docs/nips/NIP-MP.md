...
**Mode 2 — `until` only (boundary-bucket drain; exhaustive only under a relay short-response guarantee).** A vanilla NIP-01 filter offers no id tiebreak, so the only cursor is `until`. Neither naive step is safe: `until = oldest_seen_created_at - 1` skips every unread event in that second, and `until = oldest_seen_created_at` re-requests the whole bucket, which never advances once one `created_at` bucket exceeds the relay's page size. A mode-2 client MUST therefore drain the boundary second explicitly before stepping past it.

**The guarantee this mode rests on.** Every exhaustion inference below reads a short response as "nothing further matches" — and that is a property of the relay, not of NIP-01, where `limit` is advisory: relays "SHOULD use the `limit` value to guide how many events are returned in the initial response. Returning fewer events is acceptable" (NIP-01). A conforming relay may answer a request for 100 with 50 events and no indication that it withheld the rest, and the client cannot tell that from exhaustion. This mode is exhaustive only on a relay that **both** (1) applies the **complete filter before enforcing any limit** — a relay that post-filters after limiting can return a short (even empty) response while older matching events sit beyond the limited window, so short responses carry no exhaustion signal there — and (2) **exposes the exact effective page limit it enforces**, so the client can evaluate "shorter than the effective limit": the effective page limit is the smaller of the requested `limit` and any cap the relay imposes, since a clamped request answered in full is short without being exhausted, but if the advertised cap differs from the enforced one, "shorter than the effective limit" is undecidable by the client. A relay satisfying only one of these conditions does not provide the guarantee. Absent it, a mode-2 client MUST mark the collection possibly incomplete regardless of any response size, and the drain below serves to reduce silent loss rather than to eliminate it. `limit` below means the effective page limit.
...
Enumeration is therefore exhaustive when the relay makes the short-response guarantee above and every equal-`created_at` bucket fits in one response; under those conditions truncation is detected exactly rather than guessed at. On detecting it — or on any relay that does not make the guarantee — a client MUST mark the collection as possibly incomplete rather than present a partial collection as complete. Silently presenting a truncated collection is the failure this NIP exists to prevent: a repository missing from the list is indistinguishable from one that was never announced.
