...
Rotation or recovery requires a separate privileged transition. Replacing `k_old` with `k_new` requires explicit administrative or documented recovery authorization, an active `(i, k_old)` binding or pending-replacement record for that pair, an identity `i` that is not disabled, no active binding for `k_old` other than the active `(i, k_old)` case, and no active binding or lifecycle gate for `k_new`. Any key claim supplied to the transition MUST come from a fresh assertion and MUST equal `k_new`; where the domain requires issuer attestation, such a key claim MUST be supplied. The transition atomically retires the old pair and key, creates `(i, k_new)` with `attested-key` provenance when a fresh matching key claim is present and `provisioned` provenance otherwise, clears the pending-replacement state, records durable lifecycle history, and invalidates leases for `k_old`. A routine request presenting a new key is either a binding conflict or `explicit replacement required` and MUST be denied without mutation.
...
Base V1 has no standalone operation that merely clears a disabled-identity selector. Re-enablement requires a privileged `EnableIdentity` transition that selects an eligible replacement or first key and atomically creates its binding, clears the disabled and pending-replacement selectors, appends lifecycle history, and revokes any prior key recorded by pending replacement. A prior key recorded by pending replacement MUST have no active binding before re-enablement can revoke it. Any key claim supplied to the transition MUST come from a fresh assertion and MUST equal the selected key; where the domain requires issuer attestation, such a key claim MUST be supplied. A matching fresh key claim records `attested-key` provenance; otherwise the privileged transition records `provisioned`. For a never-enrolled identity there is no prior key to revoke, but the new binding is still created in the same transaction that clears disablement. Ordinary enrollment therefore never observes a re-enabled identity with neither an active binding nor a lifecycle gate.
