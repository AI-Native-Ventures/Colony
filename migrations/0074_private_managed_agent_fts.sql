-- NIP-PMA kind:30194 carries the owner's private managed-agent payload
...
BEGIN
    ...
    EXECUTE format(
        'ALTER TABLE events ADD COLUMN search_tsv TSVECTOR GENERATED ALWAYS AS (CASE WHEN kind = 30194 THEN NULL::tsvector ELSE (%s) END) STORED',
        existing_expression
    );
