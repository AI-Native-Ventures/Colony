...
| `buzz-cli` (agent-first, JSON in / JSON out) + ACP harness (Oh My Pi, OpenCode, Codex, Claude Code, Colony Agent) | Huddle lifecycle events | Culture features |
...
**Agent surface** — `buzz-cli` (agent-first CLI, JSON in / JSON out) · `buzz-acp` (ACP harness for Oh My Pi / OpenCode / Codex / Claude Code / Colony Agent) · `buzz-agent` (ACP agent — see [VISION_AGENT.md](VISION_AGENT.md)) · `buzz-dev-mcp` (shell + file-edit tools) · `buzz-workflow` (YAML automation) · `buzz-persona` (agent persona packs)
