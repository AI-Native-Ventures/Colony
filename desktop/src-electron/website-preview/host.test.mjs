...
test("production selects the wrapper only on macOS", () => {
...
test("open mounts one isolated sandboxed view per identity", async () => {
  ...
  await host.open(requestFor(window, { artifactId: "2".repeat(64) }));
...
test("owner window close tears down only that window's previews", async () => {
  ...
  await host.open(requestFor(windowB, { artifactId: "2".repeat(64) }));
...
test("community invalidation detaches synchronously then cleans up", async () => {
  ...
  await host.open(requestFor(window, { artifactId: "2".repeat(64) }));
  await host.open(
    requestFor(window, { artifactId: "3".repeat(64), communityId: "community-2" }),
  );
...
test("view count and memory budgets fail closed", async () => {
  ...
  await assert.rejects(
    single.host.open(requestFor(window, { artifactId: "2".repeat(64) })),
  ...
  await assert.rejects(
    sized.host.open(requestFor(window, { artifactId: "2".repeat(64) })),
