...
try {
  ...
  if (liveWebsite && page && !page.isClosed()) {
    ...
    proof.websiteAgentDiagnostics = await page
      .evaluate(async () => {
        ...
        return Promise.all(
          agents.map(async (agent) => {
            const log = await invoke("get_managed_agent_log", {
              pubkey: agent.pubkey,
              lineCount: 40,
            }).catch(() => null);
            ...
            return {
              ...
              authenticationError:
                /unauthorized|authentication failed|invalid.*key|\b401\b/i.test(
                  content,
                ),
              creditError:
                /insufficient.*credit|budget.*exceeded|\b402\b/i.test(content),
              connectionError:
                /connection refused|connection failed|dns|timed out/i.test(
                  content,
                ),
              modelError: /model.*not found|unsupported model|\b404\b/i.test(
                content,
              ),
