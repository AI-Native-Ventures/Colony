...
const post = (provider, body = {}, token = provider.token) => fetch(
...
test("unauthorized requests never call the paid upstream", async () => {
  let calls = 0;
  const provider = await createLiveProofProvider({ apiKey: key, fetchImpl: async () => {
    calls += 1;
    return completion();
  } });
  try {
...
test("parallel teammates serialize; client cannot change model or forward upstream options", async () => {
  let active = 0;
  let peak = 0;
  const provider = await createLiveProofProvider({ apiKey: key, fetchImpl: async (url, options) => {
    active += 1;
  ...
  try {
    const results = await Promise.all([post(provider, { model: "other", max_tokens: 999999, api_key: "bad" }), post(provider)]);
...
test("a provider failure stops queued and later paid requests without echoing secrets", async () => {
  ...
  try {
    ...
    assert.deepEqual(results.map((result) => result.status), [502, 429]);
