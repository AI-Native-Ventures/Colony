 ...
 */
const NO_PREVIEW_ACTIVITY: ReadonlySet<string> = new Set();

export function useSidebarUnreadOverflow({
  dmChannelIds,
  highPriorityUnreadChannelIds,
  // Colony never took the agent/activity overflow projection this unions in,
  // and #7134 removes it upstream too, so it defaults to empty here.
  previewActivityChannelIds = NO_PREVIEW_ACTIVITY,
  scrollRef,
  unreadChannelIds,
}: {
  dmChannelIds: ReadonlySet<string>;
