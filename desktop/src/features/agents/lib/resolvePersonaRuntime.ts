 ...
 * order as the shipped adapter list: Oh My Pi first (the signup default),
 * then the bundled buzz-agent sidecar, then the first available entry, then
...
export function getDefaultPersonaRuntime<T extends AcpRuntimeCatalogEntry>(
  ...
  return (
    ...
    available.find((runtime) => runtime.id === "omp") ??
