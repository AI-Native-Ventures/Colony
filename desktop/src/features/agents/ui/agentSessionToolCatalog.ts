...
export function getBuzzToolInfo(title: string): BuzzToolInfo | null {
  ...
  if (name.includes("workflow") || name === "approve_step") {
    return {
      icon: Workflow,
      label: isRead
        ? "Reads workflow state from Colony."
        : "Updates workflow state in Colony.",
  ...
  if (
    ...
    return {
      icon: Hash,
      label: isRead
        ? "Reads channel context from the Colony relay."
        : "Changes channel state in the Colony relay.",
      tone: isWrite ? "write" : "read",
    };
  }
  if (
  ...
  if (name.includes("search") || name === "get_feed") {
    return {
      icon: Search,
      label: "Searches relay-visible Colony history.",
      tone: "read",
    };
  }
  if (
    ...
    return {
      icon: Send,
      label: "Publishes relay-visible Colony activity.",
      tone: "write",
    };
  }
  ...
  return {
    icon: MessageSquare,
    label: isRead ? "Reads from Colony." : "Writes to Colony.",
    tone: isWrite ? "write" : "read",
  };
}
