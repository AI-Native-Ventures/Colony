...
registerHooks({
  resolve(specifier, context, nextResolve) {
    ...
    // Colony's UserProfilePopover reads the huddle and community contexts in
    // its wrapper, where upstream defers them to the opened panel, so rendering
    // a row would need the whole app shell.
    if (specifier === "@/features/profile/ui/UserProfilePopover") {
      return { shortCircuit: true, url: "buzz-inbox-stub:UserProfilePopover" };
    }
    return nextResolve(specifier, context);
  },
  load(url, context, nextLoad) {
    if (url === "buzz-inbox-stub:UserProfilePopover") {
      return {
        format: "module",
        shortCircuit: true,
        source:
          "export const UserProfilePopover = ({ children }) => children;\n",
      };
    }
...
Object.assign(globalThis, {
  ...
  // Colony's detail pane pulls in a dependency that reads bare `location` at
  // module load, which upstream's harness never needed.
...
// Colony routes native calls through the NativeBridge seam, so the stub above
// is never consulted on its own: install it as the bridge.
before(async () => {
