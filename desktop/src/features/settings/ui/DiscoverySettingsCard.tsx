...
import {
  deleteDiscoveryOutscraperCredential,
  getDiscoveryOutscraperCredentialStatus,
  saveDiscoveryOutscraperCredential,
...
export function DiscoverySettingsCard() {
  ...
  React.useEffect(() => {
    ...
    void getDiscoveryOutscraperCredentialStatus()
      .then((nextStatus) => {
  ...
  async function handleSave(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const nextStatus = await saveDiscoveryOutscraperCredential(value);
      setStatus(nextStatus);
      setValue("");
      setShowValue(false);
      setConfirmation("Outscraper is connected on this device.");
    } catch {
      setError(
        "The API key could not be saved securely. Nothing was sent to Outscraper.",
      );
    } finally {
      setIsSaving(false);
    }
  ...
  async function handleDelete() {
    ...
    setConfirmation(null);
    try {
      const nextStatus = await deleteDiscoveryOutscraperCredential();
      setStatus(nextStatus);
      setValue("");
      setShowValue(false);
      setConfirmation("Outscraper was disconnected from this device.");
    } catch {
      setError(
  ...
  return (
    <section className="min-w-0" data-testid="settings-discovery">
      ...
      <SettingsOptionGroup>
        <SettingsOptionRow className="items-start py-4">
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-medium">Outscraper Businesses</p>
              <p className="mt-0.5 text-sm text-muted-foreground">
                Finds public business listings from Google Maps. People
                discovery and contact enrichment are not included in this phase.
              </p>
            </div>
          </div>
          {statusContent ? (
            <span
              ...
              data-testid="discovery-credential-status"
            >
              {statusContent.label}
            </span>
          ) : (
            <span
              ...
              data-testid="discovery-credential-loading"
            >
        ...
        <SettingsOptionRow className="flex-col items-stretch justify-start gap-3 border-t border-border/40 py-4">
          <div className="flex items-start gap-2.5 rounded-xl border border-emerald-500/20 bg-emerald-500/5 px-3 py-2.5">
            ...
            <p className="text-sm text-muted-foreground">
              Your key stays in this device&apos;s secure credential store.
              Colony does not upload or synchronize it. Live runs are billed
              directly to your Outscraper account; saving a key does not start a
              run.
            </p>
          </div>
          ...
          {storageUnavailable ? (
            <div
              ...
              data-testid="discovery-credential-unavailable"
            >
              <TriangleAlert className="mt-0.5 h-4 w-4 shrink-0" />
              Unlock your system keychain and restart Colony before connecting
              Outscraper.
            </div>
          ) : (
            <form className="space-y-3" onSubmit={handleSave}>
              <label
                ...
                htmlFor="discovery-outscraper-key"
              >
                {status === "configured"
                  ? "Replace API key"
                  : "Outscraper API key"}
              ...
              <div className="flex flex-col gap-2 sm:flex-row">
                <div className="relative min-w-0 flex-1">
                  <input
                    ...
                    data-testid="discovery-outscraper-key-input"
                    disabled={isBusy || status === null}
                    id="discovery-outscraper-key"
                    onChange={(event) => {
                ...
                <Button
                  data-testid="discovery-save-credential"
                ...
                {status === "configured" ? (
                  <Button
                    data-testid="discovery-delete-credential"
                    disabled={isBusy}
          ...
          {confirmation ? (
            <p
              ...
              data-testid="discovery-credential-confirmation"
            >
          ...
          {error ? (
            <p
              ...
              data-testid="discovery-credential-error"
            >
              <TriangleAlert className="mt-0.5 h-4 w-4 shrink-0" />
              {error}
            </p>
