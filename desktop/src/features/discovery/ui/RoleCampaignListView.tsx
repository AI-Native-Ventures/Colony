...
import type { CampaignSummary, ProfessionalRoleDetail } from "../types";
import { campaignProgressPercent } from "./discoveryLayout";

export type RoleCampaignListViewProps = {
  role: ProfessionalRoleDetail;
...
export function RoleCampaignListView({
  role,
  campaigns,
  fieldName,
  onBack,
  onOpenCampaign,
  onCreateCampaign,
}: RoleCampaignListViewProps) {
  const totalPeople = campaigns.reduce(
  ...
  return (
    <>
      <button
        aria-label="Close role campaign sidebar"
        className="fixed inset-0 z-40 cursor-default bg-black/50"
        onClick={onBack}
        type="button"
      />
      <aside
        aria-label={`${role.name} campaigns`}
        ...
        data-testid="discovery-role-campaign-sidebar"
        role="dialog"
      >
        <div className="relative h-48 shrink-0 overflow-hidden">
          <img
            alt={fieldName ?? role.name}
        ...
        <div className="space-y-8 px-6 pb-10 pt-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Button
                data-testid="create-people-discovery-campaign"
            ...
            {campaigns.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-border p-8 text-center">
                <p className="text-sm text-muted-foreground">
                  No campaigns yet for this role.
                </p>
                <Button
              ...
              campaigns.map((campaign) => {
                ...
                return (
                  <button
                    ...
                    data-testid={`discovery-people-campaign-card-${campaign.id}`}
                    key={campaign.id}
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 line-clamp-2 text-sm text-muted-foreground">
                          {campaign.description ?? "People discovery campaign"}
                        </p>
                      </div>
