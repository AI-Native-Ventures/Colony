...
// ── Test: an edit delivered with the thread page renders as the reply body ────
//
// The reason the thread fetch no longer backfills its own aux closure (#6572
// added `include_aux` to the relay's thread-subtree read, so edits, deletions
// and reactions arrive on the same page as the replies). This drives the case
// the aux backfill used to cover: a reply edited BEFORE the thread is opened.
// If the relay's aux events were dropped on the way through
// `loadThreadReplies`, the panel would render the original text.

test("a reply edited before the thread opens renders its edited body", async () => {
  const { loadThreadReplies } = await import("./useThreadReplies.ts");
  ...
  // The edit the relay hands back alongside the replies, authored before the
  // thread was ever opened.
  const edit = {
    id: editId,
    pubkey: author,
    kind: 40003,
  ...
  const fetched = await loadThreadReplies(
    makeQueryClient(),
    channelId,
    rootId,
    null,
    new Set(),
    async () => singlePage([reply, edit]),
  );

  assert.ok(
    fetched.some((event) => event.id === editId),
    "the relay's aux closure must survive the thread fetch",
  );

  const rendered = formatTimelineMessages(fetched, null, undefined, null);
  const replyRow = rendered.find((message) => message.id === replyId);
  assert.ok(replyRow, "the edited reply must still render");
  assert.equal(replyRow.body, "these PRs?");
  assert.equal(replyRow.edited, true);
});
