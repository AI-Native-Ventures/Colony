...
export function useComposerVoiceNote({
  ...
  const acceptsAttachment = recorder.status === "idle" && !hasAttachment;
  // Mirrored for the paste handler, which reads it inside a Tiptap prop that
  // is installed once rather than re-created per render.
  const acceptsAttachmentRef = React.useRef(acceptsAttachment);
  acceptsAttachmentRef.current = acceptsAttachment;
  // The drop zone lives a level up (ChannelPane owns the main column), so a
  // recording in progress has to withdraw its acceptance from here.
  React.useEffect(() => {
    onAttachmentAcceptanceChange?.(acceptsAttachment);
  }, [acceptsAttachment, onAttachmentAcceptanceChange]);
