...
// Colony's UserProfilePopover reads the active community and the router from
// its wrapper, where upstream defers both to the opened panel. These cases
// assert membership copy, so the popover renders as its bare trigger here.
mock.module("@/features/profile/ui/UserProfilePopover", {
  namedExports: {
    UserProfilePopover: ({ children }) => children,
  },
});
