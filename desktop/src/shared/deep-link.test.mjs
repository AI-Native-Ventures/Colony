...
// Colony routes native calls through the NativeBridge seam rather than
// `__TAURI_INTERNALS__`, so the upstream stub above is not enough on its own:
// install a bridge backed by the same handler map. `listen` goes through the
// same map so the unlisten count this file asserts on still moves.
...
const mockBridge = createMockNativeBridge((command, args) => {
...
setNativeBridge({
  ...
  listen: async (event, callback) => {
    const id = nextCallbackId++;
    callbacks.set(id, callback);
    ipcHandlers.get("plugin:event|listen")?.({ event });
    return async () => {
      callbacks.delete(id);
      ipcHandlers.get("plugin:event|unlisten")?.({ event });
    };
  },
});
