...
test("relayClientSession wires closeSubscription into handleRelayClosed", async () => {
  ...
  // Colony dispatches inbound frames through `relayClientInbound`, so the
  // handleRelayClosed call that must carry the callback lives there.
  const inbound = await readFile(
    new URL("./relayClientInbound.ts", import.meta.url),
    "utf8",
  );
  assert.match(
    inbound,
    /handleRelayClosed\(\{[^}]*closeSubscription[^}]*\}\)/,
    "relayClientInbound.ts must pass closeSubscription to handleRelayClosed",
  );
