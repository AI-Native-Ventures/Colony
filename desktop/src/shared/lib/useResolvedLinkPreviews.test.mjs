...
before(() => {
  ...
  // `window.__TAURI_INTERNALS__`, so the per-test handler map is installed as a
  // mock bridge. Each test still controls its own fetch resolution timing.
  setNativeBridge(
    createMockNativeBridge((cmd, args) => {
      const handler = ipcHandlers.get(cmd);
      return handler
        ? handler(args)
        : Promise.reject(new Error(`unmocked native command: ${cmd}`));
    }),
  );
});
