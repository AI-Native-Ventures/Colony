...
// mediaUrl starts its proxy-port poll at module load only when a `window`
// global exists (the Tauri renderer environment). Node has none, so alias
// globalThis the same way the other bridge-backed tests do. The OLD mock
// stubbed `globalThis.window = { __TAURI_INTERNALS__ }` per test, which
// happened to satisfy this check; the NativeBridge mock must do it explicitly.
...
test("media-proxy port store: resolved port publishes and reset notifies subscribers", async () => {
  let notifications = 0;
  // The module-load poll races the subscribe below, so gate the port behind a
  // deferred and resolve it only after the subscription is registered — the
  // assertion is about notify-on-publish, not about poll-vs-subscribe order.
  const port = deferred();

  setNativeBridge(
    createMockNativeBridge((command) => {
      if (command === "get_media_proxy_port") return port.promise;
