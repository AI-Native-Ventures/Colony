...
// The preview server's port is configurable (`PLAYWRIGHT_PORT`), so the fixture
// URL is derived from the config rather than pinned to 4173: a wrong port makes
// the media element fire `error` and the card renders "Audio unavailable"
// before any assertion about the waveform runs.
const AUDIO_URL = `http://127.0.0.1:${process.env.PLAYWRIGHT_PORT ?? "4173"}/sounds/ping.mp3`;
...
test("restores the voice note and shows a send error after upload succeeds", async ({
  ...
  await expect(composerCard).toBeVisible();
  await expect(
    // Colony's finish-send handler toasts the failure itself, without
    // upstream's "Message failed to send:" prefix (`createFinishSendFailureHandler`).
    page.getByText("relay rejected voice note"),
  ).toBeVisible();
});
...
test("generic audio retains the relay-native download action", async ({
  ...
  // Audio splits by kind: only a voice-note descriptor renders upstream's
  // attachment surface, so a generic mp3 stays on Colony's `AudioPlayer`
  // (`media-audio-preview`). The download action under test is the same
  // control with the same accessible name.
  const card = page.getByTestId("media-audio-preview").last();
...
// Upstream's case for this ("keeps emoji available but blocks GIFs beside a
// queued voice note") is not ported: it drives a GIF tab inside the emoji
// picker, which arrives with an unported commit - Colony's picker has no GIF
// surface, so its button is "Insert emoji" and there is no tab to disable.
