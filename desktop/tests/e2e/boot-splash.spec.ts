...
// loading gate keeps the walking ant up as an overlay above the already
...
test("boot splash overlay holds with a walking ant, then dismisses", async ({
  page,
}) => {
  ...
  // The ant's legs are actually animating while the overlay holds — pure CSS, no SMIL.
  const legState = await overlay.locator(".ant-leg-layer-a").evaluate((leg) => {
    const animation = leg.getAnimations()[0];
    return {
      name: getComputedStyle(leg).animationName,
      state: animation?.playState,
    };
  });
  expect(legState).toEqual({ name: "ant-step-a", state: "running" });
