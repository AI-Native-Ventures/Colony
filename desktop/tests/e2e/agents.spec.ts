...
test("catalog hides built-ins and shows the shared-agent empty state", async ({
  ...
  for (const personaName of STARTER_NAMES) {
  ...
  await page
    ...
    .getByRole("button", { name: "Close" })
    .click();
  await page.getByLabel(`Open actions for ${GUIDE_NAME}`).click();
  await page.getByRole("menuitem", { name: "Share" }).click();
...
test("built-in persona edits persist", async ({ page }) => {
  ...
  >(page, "list_personas");
  expect(
    personas.find((persona) => persona.id === STARTER_PERSONA_IDS.fizz),
  ).toMatchObject({
...
test("team cards use the thread-style overlapping avatar stack", async ({
  ...
  await installMockBridge(page, {
    personas: [
      {
        avatarUrl: starterPersonaAnimation(STARTER_PERSONA_IDS.fizz),
        id: personaIds[0],
        displayName: "Design",
...
test("inactive built-ins cannot be used to create teams", async ({ page }) => {
  ...
  const error = await invokeTauriExpectError(page, "create_team", {
    input: {
      name: "Honeys",
      personaIds: [STARTER_PERSONA_IDS.honey],
    },
  });

  expect(error).toBe(`${SECOND_NAME} is not in My Agents.`);
});

test("built-in removal failures show up from My Agents", async ({ page }) => {
  await installMockBridge(page, {
    activePersonaIds: [STARTER_PERSONA_IDS.honey],
  });
  await gotoApp(page);
  ...
  await page.getByLabel(`Open actions for ${SECOND_NAME}`).click();
  await page.getByRole("menuitem", { name: "Delete" }).click();

  await expect(
    page
      .locator("[data-sonner-toast]")
      .filter({ hasText: `${SECOND_NAME} is still referenced by a team.` }),
  ).toBeVisible();
});
