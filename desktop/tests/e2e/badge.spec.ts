...
/**
 * Colony's mock workspace starts with unread rooms of its own, so the absolute
 * numeral on the overflow pill is not upstream's 1. The claim #7134 makes is
 * about destinations: reading the count and asserting it does not move is the
 * same proof without pinning Colony's seed.
 */
async function unreadOverflowCount(
  pill: import("@playwright/test").Locator,
): Promise<number> {
  const label = (await pill.getAttribute("aria-label")) ?? "";
  // The DM-targeted label prefixes the sender, so the count is not at the start.
  const match = label.match(/(\d+) unread/);
  if (!match) throw new Error(`Overflow pill has no count: ${label}`);
...
test("offscreen unread counts destinations and promotes without incrementing", async ({
  ...
  const destinationsBefore = await unreadOverflowCount(activityArrow);
  ...
  expect(await unreadOverflowCount(activityArrow)).toBe(destinationsBefore);
  await waitForAnimations(page);
  await page.screenshot({
...
test("offscreen unread DM shows the primary sidebar arrow", async ({
  ...
  await expect(activityArrow).toBeVisible();
  expect(await unreadOverflowCount(activityArrow)).toBeGreaterThan(0);
  await expect(activityArrow).toHaveClass(/bg-primary/);
});
...
test("top-level @mention bolds its channel without a trailing numeral", async ({
  ...
  await expect(page.getByTestId("channel-unread-random")).toHaveCount(0);
  await expect(page.getByTestId("channel-unread-dot-random")).toHaveCount(0);
  // #7134 keeps the Dock badge separate from sidebar emphasis, and Colony's
  // Dock counts a mention once where upstream's counts it twice.
