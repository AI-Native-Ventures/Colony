...
test("creates the DM before preparing a persona mention", async ({ page }) => {
  ...
  await expect(page.getByTestId("chat-title")).toContainText(GUIDE_NAME);
...
test("routes an agent mention from an existing DM to the expanded conversation", async ({
  ...
  await expect(page.getByTestId("chat-title")).toContainText(GUIDE_NAME);
  await expect(sourceDm).not.toContainText(GUIDE_NAME);
...
test("drops an expanded DM after agent startup fails", async ({ page }) => {
  ...
  await expect(page.getByTestId("chat-title")).not.toContainText(GUIDE_NAME);
});
...
test("create ephemeral stream shows sidebar and header affordances", async ({
  ...
  // Colony renders a solid unread dot on channel rows rather than a count (#1253).
  await expect(
    page.getByTestId(`channel-unread-dot-${channelName}`),
  ).toBeVisible();
  await expect(
    page.getByTestId(`channel-ephemeral-${channelName}`),
  ).toHaveCount(0);
