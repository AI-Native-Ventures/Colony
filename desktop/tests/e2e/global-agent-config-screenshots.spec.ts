...
test.describe("global agent config screenshots", () => {
  ...
  test("create exposes setup guidance when no harness is available", async ({
    ...
    await selectDropdownOption(page, harness, "Colony Agent (not installed)");
    await expect(
      customSection
        .locator("p")
        .filter({ hasText: "Colony Agent is not installed." }),
    ).toContainText(
      "Colony Agent is not installed. Ships with the Colony desktop app. Visit Settings > Agents to set it up.",
    );
    await expect(page.getByTestId("persona-dialog-submit")).toBeDisabled();
  });
