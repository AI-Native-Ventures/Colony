...
import { clickTestId, waitForFirstPaint, waitForTestId } from "../helpers/app";
import {
  processTree,
  waitForPidsGone,
  waitForProcessWhere,
...
describe("07 packaged workspace terminal SIGTERM", () => {
  it("reaps every live terminal process when the app receives SIGTERM", async () => {
    recordResult("07-workspace-terminal-sigterm", "pass", "running");
    ...
    await sendTerminalCommand("sleep 30 & printf 'terminal-sigterm-output\\n'");
    ...
    await sendTerminalCommand("sleep 30 & printf 'terminal-sigterm-second\\n'");
    ...
    recordResult("07-workspace-terminal-sigterm", "pass", `appPid=${app.pid}`);
  });
});
