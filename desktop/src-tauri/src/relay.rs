...
///
/// The per-record `relay_url` pin wins whenever it is set. A community is a
/// hard boundary: an agent created in one community must never run, publish,
/// or appear in another, so the community it was created in is the community
/// it belongs to for the rest of its life. This is the one choke point every
/// agent relay resolution flows through — reconcile, spawn, and profile sync
/// all land here — so honoring the pin here is what makes the boundary hold.
///
...
pub fn effective_agent_relay_url(record_relay: &str, workspace_relay: &str) -> String {
    ...
    buzz_core_pkg::relay::normalize_relay_url(pinned).unwrap_or_else(|_| pinned.to_string())
}
...
fn build_profile_event(
    ...
    let builder =
        crate::events::build_profile(Some(display_name), None, avatar_url, None, None, role)?;
...
pub async fn sync_managed_agent_profile(
    ...
    role: Option<&str>,     // workspace role this instance fills
) -> Result<(), String> {
    crate::relay_admission::wait_for_rate_limit().await;
    // Build a signed kind:0 profile event (with optional NIP-OA auth tag).
    let event = build_profile_event(agent_keys, display_name, avatar_url, auth_tag, role)?;
...
pub struct AgentProfileInfo {
    ...
    /// Workspace role advertised by the published profile, if any.
...
#[cfg(test)]
#[path = "relay/profile_role_tests.rs"]
mod profile_role_tests;

#[cfg(test)]
mod tests {
    use super::{
        agent_belongs_to_workspace, build_profile_event, classify_intercepted_response,
