...
#[test]
fn test_document_preview_fixtures_pass_native_download_content_policy() {
    let csv = include_bytes!("../../../public/rich-previews/service-ledger.csv");
    let workbook = include_bytes!("../../../public/rich-previews/service-ledger.xlsx");
    let pdf = include_bytes!("../../../public/rich-previews/service-report.pdf");
    assert_eq!(
        detect_and_validate_mime(csv).as_deref(),
        Ok("application/octet-stream")
    );
    assert_eq!(
        detect_and_validate_mime(pdf).as_deref(),
        Ok("application/pdf")
    );
    assert!(matches!(
        detect_and_validate_mime(workbook).as_deref(),
        Ok("application/zip")
