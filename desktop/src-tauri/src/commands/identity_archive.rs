...
/// [`fetch_relay_self_at`] and the Discovery capability probe so the two reads
/// cannot drift in URL, header, or parse handling.
async fn fetch_relay_information_document_at(
    state: &AppState,
    relay_url: &str,
...
/// [`fetch_relay_information_document_at`] against the active workspace relay.
async fn fetch_relay_information_document(
    state: &AppState,
) -> Result<Option<RelayInformationDocument>, String> {
    fetch_relay_information_document_at(state, &relay_ws_url_with_override(state)).await
}
...
pub(crate) async fn fetch_relay_self_at(
    ...
    let Some(doc) = fetch_relay_information_document_at(state, relay_url).await? else {
        return Ok(None);
    };
