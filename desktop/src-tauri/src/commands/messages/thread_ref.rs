//! Thread-reference resolution for message sends.
//!
//! Split out of `messages.rs` alongside upstream's collapsed reply send: when
//! the renderer already holds the thread root (it rendered the parent), it
//! passes it down and this module skips the relay round trip entirely.
...
/// Use the renderer-supplied root when there is one, else resolve it.
pub(super) async fn thread_ref(
    ...
    match root_event_id {
        ...
        None => resolve_thread_ref(parent_event_id, state).await,
    }
}
