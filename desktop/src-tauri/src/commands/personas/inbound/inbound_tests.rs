...
use super::*;
// The team fixtures live with the team tests; the cross-kind tombstone test
// below removes a team alongside a persona and an agent.
...
fn inbound_managed_agent_drops_injected_secrets_and_harness() {
    ...
    // The head arrives on the agent's OWN community, so the pin guard lets it
    // through and these assertions see the apply itself.
