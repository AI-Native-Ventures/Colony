...
/// Delegates to the shared engine in `reconcile`, applying the same
/// record-scoped retention fix.
...
// Test modules live in sibling files (`import_egress_guard_tests.rs`,
// `import_avatar_tests.rs`) for the file-size discipline; the module paths
// (`import::egress_guard_tests`, `import::import_avatar_tests`) are unchanged.
#[cfg(test)]
#[path = "import_egress_guard_tests.rs"]
