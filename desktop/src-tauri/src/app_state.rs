    ...
    pub fn signing_keys(&self) -> Result<Keys, String> {
                ...
                .load(std::sync::atomic::Ordering::Acquire)
        {
            return Err("identity is in recovery mode; event signing is disabled \
                 until the identity is restored and Colony is relaunched"
                .to_string());
        }
        self.keys
            .lock()
...
fn load_or_create_identity(data_dir: &std::path::Path) -> Result<ResolvedIdentity, String> {
    ...
    // A channel-scoped build (Canary) may be booting for the first time since
    // its service was renamed away from `"buzz-desktop"` — see
    // `identity_resolution` for why this must be checked before generating.
    let legacy_store =
        keyring_config::legacy_keyring_service().map(crate::secret_store::SecretStore::keyring);
    identity_resolution::resolve_identity_with_stores(
        store,
        legacy_store.as_ref(),
        &legacy_path,
        data_dir,
...
#[path = "app_state_identity_resolution.rs"]
mod identity_resolution;
#[cfg(test)]
pub(crate) use identity_resolution::resolve_identity_with_store;
